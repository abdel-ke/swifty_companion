...
class BGClusterStage extends StatelessWidget {
  const BGClusterStage({super.key, required this.index});
  final int index;
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: fetchClusters(BGClustersSvgUrl[index], BENGURIRID),
  ...
  Positioned _buildTextPositioned(dynamic e, List<dynamic> viewBoxValues,
    ...
    List lastTwoNumbers = [];
    if (e["transform"] == null) {
      if (e["font-size"] == "20" || e["font-size"] == 20) {
        debugPrint('x: ${e["x"]}, y: ${e["y"]}');
        lastTwoNumbers = [e["x"], e["y"]];
      }
    } else {
      lastTwoNumbers = getLastTwoNumber(e["transform"]);
    }
    if (lastTwoNumbers.isEmpty) {
